...
desire_dict = {
    ...
    SWAPMODEL: [
        DUETHOLDANDFARM, DASSETHOLDANDFARM, SHORTFARMARBITRAGE, SPREAD, BULLISHDASSET, BULLISHDUET, BEARISHDUET,
        BEARISHDASSET, DUETHOLD
    ],
...
class MBMExperiment(Experiment):
    def __init__(self, file_path='', start_reward_day=0, trade_limit=500, price_change=False, no_reward=False,
                 low_liquity=False, high_gas=False, different_ratio=[0.1, 0.2, 0.3, 0.3, 0.1], init_token=5000,
                 code=[], market_type='lmsr'):
        ...
        self.desire_list = [DUETHOLDANDFARM, DUETHOLD] * different_ratio[0] + [DASSETHOLDANDFARM, DASSETHOLD,
                                                                               SHORTFARMARBITRAGE] * different_ratio[
                               1] + [BULLISHDASSET, BULLISHDUET] * different_ratio[2] + [BEARISHDASSET, BEARISHDUET] * \
                           different_ratio[3] + [SPREAD] * different_ratio[4] + [DUETHOLD]
    ...
    def init_experiment(self):
        ...
        for _ in range(10):
            operate_amount = self.get_operate_amount()
            agent = AgentFactory.create_agent(self.next_id(), states, self.duet_system, HOLDFOUNDER,
        ...
        for _ in range(10):
            ...
            agent = AgentFactory.create_agent(self.next_id(), states, self.duet_system, SELLFOUNDER,
                                              operate_amount=operate_amount, founder_sell_time=sell_time,
                                              bearish_stand_drop=bearish_stand_drop)
    ...
    def set_price_changing(self):
        ...
        # medium_high = []
        # medium_low = [] 
        very_high = list(range(1, 7)) + list(range(31, 37)) + list(range(61, 67)) + list(range(91, 97)) + list(
            range(121, 127)) + list(range(151, 157))
        very_low = list(range(181, 187)) + list(range(211, 217)) + list(range(241, 247)) + list(range(271, 277)) + list(
            range(301, 307)) + list(range(331, 337))
        medium_high = list(range(361, 367)) + list(range(391, 397)) + list(range(421, 427)) + list(
            range(451, 457)) + list(range(481, 487)) + list(range(511, 517))
        medium_low = list(range(541, 547)) + list(range(571, 577)) + list(range(601, 607)) + list(
            range(631, 637)) + list(range(661, 667)) + list(range(691, 697))
    ...
    def compute_result(self):
        ...
        average = sum(
            [abs(x[1] - x[2]) + abs(x[3] - x[4]) + abs(x[7] - x[8]) + abs(x[9] - x[10]) for x in self.result]) / len(
            self.result)
